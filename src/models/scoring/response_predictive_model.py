...
class ResponsePredictiveModel(ScoringModelBase):
    ...
    def train(
        ...
        with Timer("train", verbosity="start+end") as train_timer:
            ...
            with Timer("sensitivity_analysis", verbosity="start+end", parent=train_timer):
                sensitivity_metrics = ResponsePredictiveModel._SensitivityAnalysis.compute(
                    self.network,
    ...
    class _SensitivityAnalysis:
        ...
        All methods operate on the predicted-representation scoring path, matching
        inference-time behaviour. Results are accuracy drops from baseline (positive =
        the modification hurts accuracy, i.e. the model relies on that component).

        A and B sides of each pair share the same prompt, so modifications are applied
        identically to both sides.
        """

        _PromptBatch = tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]
        # (emb_a [batch, d_emb], feat_a [batch, d_feat], emb_b [batch, d_emb], feat_b [batch, d_feat])

        @classmethod
        def compute(
            cls,
            network: "ResponsePredictiveModel._ResponsePredictiveNetwork",
        ...
        def _compute_accuracy(
            ...
            The device is inferred from the network parameters; dataloader tensors are
            moved accordingly.
            """
            ...
            with torch.no_grad():
                for (
                    ...
                    pred_repr_a = network.forward_predict(prompt_emb_a, prompt_feat_a, model_emb_a)  # [batch, repr_dim]
                    pred_repr_b = network.forward_predict(prompt_emb_b, prompt_feat_b, model_emb_b)  # [batch, repr_dim]
                    score_a = network.forward_score(prompt_emb_a, prompt_feat_a, pred_repr_a)  # [batch]
                    score_b = network.forward_score(prompt_emb_b, prompt_feat_b, pred_repr_b)  # [batch]
